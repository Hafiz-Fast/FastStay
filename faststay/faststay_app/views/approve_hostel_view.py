...
from faststay_app.services.approve_hostel_service import ApproveHostelService
import json
import secrets


@method_decorator(csrf_exempt, name='dispatch')
class ApproveHostelView(View):
    hostel_service = ApproveHostelService()
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            result = self.hostel_service.approve_hostel(hostel_id)

            if result == 1:
                return JsonResponse({"success": True, "message": f"Hostel ID {hostel_id} approved successfully"}, status=200)
            if result == 0:
                return JsonResponse({"success": True, "message": f"Hostel ID {hostel_id} is already approved"}, status=200)
            if result == -1:
                return JsonResponse({"error": f"Hostel ID {hostel_id} not found"}, status=404)

            return JsonResponse({"error": "Database system error during approval"}, status=500)

        except json.JSONDecodeError:
            return JsonResponse({"error": "Invalid JSON input"}, status=400)
        except Exception as e:
            print(f"Error approving hostel: {e}")
